...
Lingomate は Edge / Chrome 向けのサイドパネル型多言語翻訳拡張機能です。入力テキストの言語を Language Detector API で自動判定するか手動で選択し、Translator API で翻訳します。
...
翻訳には Translator API が利用できるブラウザ環境が必要です。Language Detector API が利用できる環境では入力言語を自動判定でき、未対応の場合でも入力言語を手動で選択して翻訳できます。未対応のブラウザでは、サイドパネル上に API が利用できないことを表示します。
